...
function Search({searchResult}) {
    ...
    return <div>
       <Header pHolder={`${location} | ${formattedStartDate} - ${formattedEndDate} | ${noOfGuest} guest`}/>
       <main className="flex mt-2 px-4">
           <section>
            <p className="text-sm">300+ Stays {range} for {noOfGuest}  guest</p>
               ...
               <div className="hidden lg:inline-flex mb-5 space-x-3 text-gray-800 whitespace-nowrap">
                   ...
                   <p className="button">Number of rooms</p>
                   <p className="button">Types of luxries</p>
                   
               </div>
               <div className="flex flex-col">
               {searchResult.map(({img,location,
               title,description,star,price,total})=>(
                   <InfoCard
                   key={img}
                   img={img}
                   location={location}
